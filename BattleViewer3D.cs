    ...
    public class BattleViewer3D : BattleViewer
    {
        ...
        internal class BattleUI
        {
            ...
            internal class LayoutDrawerController
            {
                ...
                internal void Update(bool visibility, bool forceUpdateGameContent = false, bool forceHide = false)
                {
                    if (drawer == null)
                        return;

                    if (drawer.rootVisibility != visibility)
                    {
                        if (visibility)
                        {
                            ...
                            foreach (var obj in objs)
                            {
                                ...
                                if (!string.IsNullOrEmpty(item.positionAnchorTag))
                                {
                                    ...
                                    // #12657 Make sure to hide the result level up display, etc. (If this is not done, the container animation will automatically return to the display state)
                                    if (forceHide)
                ...
                internal void LoadLayout(GameMain owner, Catalog catalog, Rom.LayoutProperties.LayoutNode layout, Rom.LayoutProperties.LayoutNode.UsageInGame usageInGame)
                {
					...
					switch (layout.Usage)
					{
						case Rom.LayoutProperties.LayoutNode.UsageInGame.BattleSkill:
                            filteringTag = layout.SkillFilteringTag;
                            break;
						case Rom.LayoutProperties.LayoutNode.UsageInGame.BattleItem:
                            filteringTag = layout.FilteringTag;
                            break;
						default:
							break;
					}

					FilterTagProperties = ItemFilter.CreateFilteringTagFromText(filteringTag);
                }
                ...
                internal void AutoSort()
                {
                    ...
                    if (battleSequenceManager == null) return;
                    ...
                    if (usageInGame == Rom.LayoutProperties.LayoutNode.UsageInGame.BattleSkill)
                    {
                        ...
                        foreach (var player in battleSequenceManager.playerData)
                        {
                            player?.Hero?.SortSkill(gameMain.catalog, sortType);
                        }
                        if (User != null)
                        {
                ...
                {
                    return drawer.GetRenderObjectWithPositionAnchorTag(tag, index, -1);
                }
        ...
        private bool isFirstDrawResult = true;
        ...
        internal BattleViewer3D(GameMain owner) : base(owner)
        {
            ...
            {
                reflectionEffect = new EffectDrawer();
                var res = owner.catalog.getItemFromGuid(owner.catalog.getGameSettings().GraphicBattleReflection) as Common.Resource.Particle;
                if (res != null)
                {
                    reflectionEffect = EffectDrawerBase.createAndLoad(res, owner.catalog);
                }
            }
        ...
        internal override void Draw(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            ui.result.Update(false, isFirstDrawResult, true);
            isFirstDrawResult = false;

            // 反射エフェクト
            // reflection effect
            if (reflectionEffectTarget.Count > 0)
            {
                reflectionEffect.update();
                foreach (var entry in reflectionEffectTarget)
                {
                    var chr = entry.mapChr;
                    reflectionEffect.drawFor3D(chr, chr, reflectionEffectTarget.Count, chr.getDirectionRadian());
                }
            }
        ...
        internal void DrawResult(bool isVisible, List<BattlePlayerData> playerData, ResultViewer.ResultProperty resultProperty)
        {
            ...
            //}
            ui.result.Update(isVisible, true, true);
        ...
        private void drawEffect()
        {
            ...
            if (!monsterEffectDrawer.isEndPlaying)
            {
                ...
                else
                {
                    ...
                    {
                        if (reflectionSourceList.ContainsKey(target))
                            skillSource = searchFromActors(reflectionSourceList[target]);

                        monsterEffectDrawer.draw((int)target.EffectPosition.X, (int)target.EffectPosition.Y, false);
                        var actor = searchFromActors(target);
                        if (monsterEffectDrawer.drawFor3D(skillSource?.mapChr, actor?.mapChr, effectDrawTargetMonsterList.Count, rot.Y))
                    ...
                    foreach (var target in effectDrawTargetPlayerList)
                    {
                        ...
                        if (targetLayoutNode != null)
                        {
                            ...
                            if (actor != null)
                            {
                                ...
                                if (playerEffectDrawer.drawFor3D(skillSource?.mapChr, actor.mapChr, effectDrawTargetPlayerList.Count, rot.Y))
                                    actor?.queueActorState(BattleActor.ActorStateType.DAMAGE_START);
            ...
            if (!defeatEffectDrawer.isEndPlaying)
            {
                ...
                else
                {
                    ...
                    foreach (var target in defeatEffectDrawTargetList)
                    {
                        ...
                        var actor = searchFromActors(target);
                        defeatEffectDrawer.drawFor3D(skillSource?.mapChr, actor?.mapChr, defeatEffectDrawTargetList.Count, rot.Y);
        ...
        private void drawEnemyInfo(List<BattleEnemyData> enemyMonsterData)
        {
            ...
            {
                var color = monster.ConditionColor;
                var currentTransparency = monster.ConditionTransparent ? 0.5f : 1.0f;
                ...
                else if (displayWindow == WindowType.CommandTargetMonsterListWindow)
                {
                    if (monster.IsSelect)
                    {
                        ...
                        color = Util.MultiplyColor(color, blinker.getColor());
                ...
                if (effect != null)
                {
                    if (effect is EffectDrawer)
                        color = Util.MultiplyColor(color, effect.getNowTargetColor());
                ...
                actor.setConditionOpacity(currentTransparency);
            ...
            foreach (BattleEnemyData monster in fadeoutEnemyList.Where(x => !enemyMonsterData.Contains(x)))
            {
                ...
                //if (!monster.monster.visibleWhenKO)
                actor.setConditionOpacity(Math.Min(1, monster.imageAlpha));
            }
        ...
        private void drawPlayerEffect(BattlePlayerData player)
        {
            ...
            var color = player.ConditionColor;
            var currentTransparency = player.ConditionTransparent ? 0.5f : 1.0f;
            ...
            friend.setConditionOpacity(currentTransparency);
        ...
        internal BattleActor AddEnemyMember(BattleEnemyData data, int index)
        {
            ...
            var actor = BattleActor.GenerateEnemy(catalog, data, index, total + 1);
            actor.source = data;
            ...
            else
            {
                switch (self.CommandReactionType)
                {
                    ...
                    case ReactionType.None:// ミスの時はこっち / Here when you make a mistake
                        ...
                        break;
                    case ReactionType.Reflection:
                        if (start)
                        {
                            if (reflectionEffectTarget.Count == 0)
                                reflectionEffect.initialize();
                            reflectionEffectTarget.Add(actor);
                        }
                        else
                        {
                            reflectionEffectTarget.Remove(actor);
                        }
                        break;
                }
            }
        ...
        private void initializeExtras(Rom.Map map)
        {
            ...
            if (extras.Count > 0 && events.Exists(x => x.UniqueGuid == extras[0].guId))
                return;
        ...
        private void prepareEnemies(List<BattleEnemyData> enemyMonsterData)
        {
            ...
            foreach (var chr in enemyMonsterData)
            {
                ...
                enemies[count] = BattleActor.GenerateEnemy(catalog, chr, count, max);
